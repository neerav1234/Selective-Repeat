...
int main()
{
    ...
    while (m < windowsize && k < totalpackets)
    {
        ...
        //scanf("%d", &acknowledgement.acknowledge[m]);                                     Chech here
        
        
        using namespace std :: chrono;
        milliseconds ms(rand()%10);

        acknowledgement.acknowledge[m]=Nack()?-1:1;
        if(ms.count() > timeout) acknowledgement.acknowledge[m]=0;
        if (acknowledgement.acknowledge[m] == -1)
        {
            repacket[j] = f1.packet[m];
            j++;
        }
        else if(acknowledgement.acknowledge[m] == 0)
        {
            repacket[j] = f1.packet[m];
            j++;
        }
