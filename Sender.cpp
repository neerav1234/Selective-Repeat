...
int main()
{
    ...
    printf("\nTotal frames or windows to be transmi:\t %d\n", (totalframes = 5));
    ...
    while (m < windowsize && n < totalpackets)
    {
        ...
        {
            printf("\nNegative acknowledgement received for pa: %d\n",f1.packet[m]);
            k = 1;
            repacket[j] = f1.packet[m];
            j++;
        }
        else if(acknowledgement.acknowledge[m] == 0){
            printf("\nTimeout occured for pa: %d\n",f1.packet[m]);
            k = 1;
            repacket[j] = f1.packet[m];
            j++;
        }
    ...
    if (k == 0)
    {
        printf("\nPositive acknowledgement received for all packets within the f: %d\n",framessend);
    }
    framessend++;
